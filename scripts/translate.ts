...
if (!bookDir) {
  console.error("Usage: bun run scripts/translate.ts <book-dir>");
  process.exit(1);
}
...
const file = Bun.file(path.join(bookDir, "sources/modern-hebrew.txt"));
const text = await file.text();
const lines = text.split("\n");

// ─── Load Strong's dictionary ───────────────────────────────────────────────
const strongsRaw = await Bun.file(path.join(projectRoot, "data/strongs-hebrew-dictionary.js")).text();
...
// ─── Load OSHB XML and build word → Strong's mapping ────────────────────────
const genXml = await Bun.file(path.join(bookDir, "sources/oshb.xml")).text();
...
const abbrev = bookConfig.oshbAbbrev.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
const verseRe = new RegExp(`<verse\\s+osisID="${abbrev}\\.(\\d+)\\.(\\d+)">([\\s\\S]*?)<\\/verse>`, "g");
let verseMatch;
...
import { stripDiacritics, toOldHebrew } from "./hebrew-utils.ts";
...
// stripDiacritics imported from hebrew-utils.ts
...
// toOldHebrew imported from hebrew-utils.ts
...
await Bun.write(path.join(bookDir, "output/old-hebrew.json"), JSON.stringify(verses, null, 2));
...
await Bun.write(path.join(bookDir, "output/vocabulary.json"), JSON.stringify(vocabulary, null, 2));
